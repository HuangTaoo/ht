...
namespace BWP.Web.Pages.B3Butchery.Reports.OutPutQuickFreezeDiffReport_
{
    public class OutPutQuickFreezeDiffReport : DFBrowseGridReportPage
    {
        DFInfo mainInfo = DFInfo.Get(typeof(ProduceOutput));
        protected override string AccessRoleName
        {
            get { return "B3Butchery.报表.产出与速冻差异表"; }
        }
        ...
        protected override void InitQueryPanel(QueryPanel queryPanel)
        {
            ...
            //显示字段包括：{会计单位}、{计划号}、{存货名称}、{存货编码}、{产出数量}（产出单存货主数量）、{速冻入库数量}（速冻入库单主数量）、{差异数量}（速冻入库数量-入库数量）
        ...
        protected override DQueryDom GetQueryDom()
        {
            ...
            query.RegisterQueryTable(typeof(tempClass), new string[] { "Department_ID", "AccountingUnit_ID", "PlanNumber_ID", "AccountingUnit_Name", "PlanNumber_Name", "Name", "Code", "OutputNumber", "QuickFreezeNumber" }, tempClass.GetQueryDom(mQueryContainer, sd, ed));
            foreach (ListItem field in checkbox.Items)
            {
                if (field.Selected)
                {
                    if (field.Value == "OutputNumber" || field.Value == "QuickFreezeNumber")
                    {
                    ...
                    {
                        query.Columns.Add(DQSelectColumn.Create(DQExpression.Sum(DQExpression.Subtract(DQExpression.IfNull(DQExpression.Field("OutputNumber"), DQExpression.Value(0)), DQExpression.IfNull(DQExpression.Field("QuickFreezeNumber"), DQExpression.Value(0)))), field.Text));
                        SumColumnIndexs.Add(query.Columns.Count - 1);
                    }
                    else
                    {
        ...
        class tempClass
        {
            ...
            public static DQueryDom GetQueryDom(QueryContainer mQueryContainer, DateInput sd, DateInput ed)
            {
                ...
                var instoreQuery = GetQuickFreezeQuery(domainID, mQueryContainer, sd, ed);
        ...
        static DQueryDom GetOutputQuery(long domainID, QueryContainer mQueryContainer, DateInput sd, DateInput ed)
        {
            ...
            query.Columns.Add(DQSelectColumn.Create(DQExpression.Value<Money<decimal>>(0m), "QuickFreezeNumber"));
        ...
        static DQueryDom GetQuickFreezeQuery(long domainID, QueryContainer mQueryContainer, DateInput sd, DateInput ed)
        {
            ...
            query.Columns.Add(DQSelectColumn.Field("Name", goodsAlias));
            query.Columns.Add(DQSelectColumn.Field("Code", goodsAlias));
            query.Columns.Add(DQSelectColumn.Create(DQExpression.Value<Money<decimal>>(0m), "QuickFreezeNumberOutputNumber"));
